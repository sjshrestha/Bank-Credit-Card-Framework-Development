...
//        rowdata = new Object[8];
    ...
    public void setBank(CreditCard card) {
        this.card = card;
    }
    ...
    private void saveAndDisplay() {
        Party customer = card.getCustomer();
        if (customer == null)
            return;
        card.getPartyController().saveParty(customer);
        IAccount currentAccount = customer.getAllAccounts().get(0);
        if (newaccount) {
            // add row to table
            rowdata[0] = customer.getName();
            rowdata[1] = currentAccount.getAccountNumber();
            rowdata[2] = "Expiry Date";
            rowdata[3] = currentAccount.getType();
            rowdata[4] = currentAccount.getCurrentBalance();
            rowdata[5] = "0";
    ...
    void JButtonDeposit_actionPerformed(ActionEvent event) {
        ...
        if (selection >= 0) {
            ...
            JDialog_Deposit dep = new JDialog_Deposit(thisframe, (String) model.getValueAt(selection, 0));
            dep.setBounds(430, 15, 275, 140);
            dep.show();
            commitTransaction(accnr, TransactionType.DEPOSIT);
            IAccount currentAccount = card.getCustomer().getAllAccounts().get(0);
            Double newamount= currentAccount.getCurrentBalance();
    ...
    void JButtonWithdraw_actionPerformed(ActionEvent event) {
        ...
        if (selection >= 0) {
            ...
            wd.show();
            commitTransaction(accnr,TransactionType.WITHDRAW);
            Double newamount= card.getAccount().getBalance();
            model.setValueAt(String.valueOf(newamount), selection, 4);
            if (newamount < 0) {
    ...
    private void commitTransaction(String accnr, TransactionType type){
        if (amountDeposit.trim() == "") return;
        card.setAccount((Account) card.getAccountController().getAccount(accnr));
        Transaction t = TransactionFactory.createTransaction(
                type,
                card.getAccount(),
                Double.parseDouble(this.amountDeposit)
        );

        Functor emailSender = EmailSenderFactory.getEmailSender(t);
        TransactionProxy tProxy = new TransactionProxy(t, emailSender);
        card.executeTransaction(tProxy);
    }
